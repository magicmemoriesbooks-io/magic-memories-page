...
FRONT_COVER = {
    "prompt": "Disney Pixar 3D style illustration. CAST: @image1 = {gender_word} adult ({age_display}), fully human body, smooth skin, two arms, two legs, preserve adult age and mature face proportions — match reference exactly. @image2 = {pet_name} ({pet_desc}): animal body, four paws, fur coat — match reference exactly. Two distinct separate beings. Exactly ONE adult + ONE pet at the campfire. SCENE: @image1 sits beside a glowing campfire, EYES OPEN, warm smile, face turned slightly toward camera so the viewer can see their expression clearly, one hand gently resting on @image2. @image2 sits right beside @image1, EYES OPEN, looking up at @image1's face with adoring devoted eyes — the pet's gaze is on the human, not on the fire. Both characters face the viewer at a slight angle, their faces clearly lit by the warm campfire glow. SETTING: Mountain campsite WIDE VIEW, epic snow-capped peaks in the background, pine trees framing the scene, warm twilight purple sky, glowing campfire between characters and viewer, tent visible in the background. ATMOSPHERE: Warm intimate bonding, golden campfire light on both faces, magical mountain evening, deep friendship. STRICT: @image1 has eyes OPEN. @image2 has eyes OPEN looking at @image1. Both faces visible and warmly lit. Centered composition for book cover. Pure illustration only, zero text or lettering. {style}"
}
