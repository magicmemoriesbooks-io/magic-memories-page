...
FURRY_LOVE_ADULT_SCENES = [
    ...
    },
    {
        ...
        "prompt": "Disney Pixar 3D style illustration. CAST: @image1 = copy exactly @image1: {gender_word} ({age_display}), adult face and mature proportions — match reference exactly. @image2 = {pet_name} ({pet_desc}): copy exactly the animal from @image2, same coat color, body size and figure — match reference exactly. Two distinct separate beings. Exactly ONE adult + ONE pet + ONE squirrel. SCENE: @image2 stands frozen on the trail, eyes wide, ears fully perked, body tense, staring at a bold chubby squirrel sitting on a low pine branch holding an acorn — a classic standoff. @image1 stands behind @image2, hand over mouth, shoulders shaking with suppressed laughter, watching the comedic standoff with pure amusement. SETTING: Forest trail WIDE VIEW, chubby squirrel on a low branch looking smug and unfazed, warm dappled forest light, pine trees in background. ATMOSPHERE: Comedic tension, wildlife encounter humor, warm playful energy, nature comedy at its best. STRICT: @image1 is standing upright on two legs. @image2 is frozen on four paws. One squirrel on branch above. {style}",
        "text_position": "split"
    },
    {
        "id": 7,
    ...
    },
    {
        ...
        "prompt": "3D illustration in the style of Disney Pixar. CAST: @image1 = exact copy of @image1: {gender_word} ({age_display}), adult face and mature proportions; exactly matches the reference. @image2 = exact copy of @image2's animal {pet_name} ({pet_desc}): same fur color, size, and build; exactly matches the reference. Two distinct beings. An adult and a pet sharing lunch. SCENE: @image1 is sitting cross-legged on the ground eating a sandwich, smiling warmly, and @image2 is sitting next to @image1 with their head gently resting on their thigh, looking at them with large, pleading, and hopeful eyes. An open backpack with food and a thermos, camping-style, is visible. There is a small treat on the ground nearby. SETTING: Mountain meadow, panoramic view. Wildflowers swaying around them. Snow-capped mountains in the background. Open backpack with food. Golden midday sun. ATMOSPHERE: Comfort and companionship, a peaceful lunch outdoors, warm midday light, a gentle breeze, the joy of sharing. REQUIREMENTS: @image1 is sitting cross-legged. @image2 is sitting next to them, with their head resting on the other person's thigh. Disney Pixar 3D style, warm and soft lighting, adventurous cinematic atmosphere, full-body shot from head to toe, characters occupy 40% of the frame, visible natural and mountainous environment, crisp illustration. REQUIREMENTS: All adults must wear appropriate hiking clothing and be fully clothed at all times.",
        "text_position": "split"
    },
    {
        "id": 12,
    ...
    },
    {
        ...
        "prompt": "Disney Pixar 3D style illustration. CAST: @image1 = copy exactly @image1: {gender_word} ({age_display}), adult face and mature proportions — match reference exactly. @image2 = {pet_name} ({pet_desc}): copy exactly the animal from @image2, same coat color, body size and figure — match reference exactly. Two distinct separate beings. Exactly ONE adult + ONE pet at the campfire. SCENE: @image1 sits on a log beside the glowing campfire, holding a stick with a marshmallow over the flames, warm firelight illuminating their face, smiling warmly, one hand offering a roasted marshmallow toward @image2. @image2 lies curled up close to the campfire on the ground, cozy and warm, eyes alert and following the marshmallow with happy anticipation, firelight reflecting in its eyes. SETTING: Campsite at night WIDE VIEW, bright crackling campfire with dancing orange flames and golden sparks rising upward, spectacular starry night sky above, tent silhouetted in background, ring of stones around the fire, pine silhouettes. ATMOSPHERE: Magical campfire warmth, starry night wonder, cozy firelight glow, deep intimate bond between human and pet. STRICT: @image1 sits on the log on two legs. @image2 lies near the fire on four paws. Campfire night scene. {style}",
        "text_position": "split"
    },
    {
        "id": 17,
    ...
    },
    {
        ...
        "prompt": "Disney Pixar 3D style illustration. CAST: @image1 = copy exactly @image1: {gender_word} ({age_display}), adult face and mature proportions — match reference exactly. @image2 = {pet_name} ({pet_desc}): copy exactly the animal from @image2, same coat color, body size and figure — match reference exactly. Two distinct separate beings. Exactly ONE adult + ONE pet walking home together. SCENE: @image1 walks down a descending mountain trail with a full backpack, peaceful and satisfied expression, looking ahead at the valley below with quiet contentment, morning light warm behind them. @image2 walks steadily beside @image1 on all four paws at a matching pace, loyal confident posture, both looking forward together, side by side as equals. SETTING: Mountain trail descending WIDE VIEW, long morning shadows on the path, valley visible far below, the silver SUV small in the distance, panoramic mountain scenery, wildflowers lining the trail, epic golden landscape. ATMOSPHERE: Fulfillment and deep gratitude, the warmth of a journey completed, golden morning light, the unbreakable bond of companions. STRICT: @image1 walks on two legs with backpack. @image2 walks beside them on four paws. Homeward journey, side by side. {style}",
        "text_position": "split"
    }
]
...
def build_pet_preview_prompt_with_photo(pet_desc: str = "", pet_species: str = "dog", pet_size: str = "medium") -> str:
    ...
    return (
        ...
        f"Convert the {animal} from @image1 into a high-quality 3D animated storybook character.{desc_hint} "
...
def build_scene_prompt(scene: dict, human_desc: str, pet_name: str, pet_desc: str, age_display: str = "30 year old adult", eye_desc: str = "", gender_word: str = "person", glasses: str = "", facial_hair: str = "", hair_color: str = "", hair_desc: str = "", **kwargs) -> str:
    ...
    if not pet_desc:
        ...
        size_labels = {"small": "small compact", "medium": "medium-sized", "large": "large big-bodied"}
        size_label = size_labels.get(pet_size, "medium-sized")
        pet_desc = f"{size_label} {animal_word}"
    ...
    if eye_color_only:
        prompt += f" EYES: {gender_word} has {eye_color_only} eyes, matching @image1 exactly."
