...
FURRY_LOVE_TEEN_SCENES = [
    {
        ...
        "prompt": "Disney Pixar 3D style illustration. HUMAN: A younger {gender_word} child ({age_display} but shown as 6 years old in a memory) with {eye_desc} eyes{glasses_desc}, small, laughing, running freely. PET: {pet_desc}, younger and energetic, running alongside the child, both joyful and carefree, matching pace. ACTION: A happy memory scene of the child and pet running together through a sunny park, pure joy and connection between them, motion blur suggesting speed and fun. SETTING: Beautiful sunny park WIDE VIEW, green grass, playground in background, warm golden afternoon light, soft dreamy edges suggesting this is a memory, slightly hazy warm filter. ATMOSPHERE: Nostalgic warmth, golden memory glow, pure happiness from the past, bittersweet beauty. STRICT: Only ONE child, ONE pet, memory/flashback scene with dreamy warm edges. {style}",
        "text_position": "split"
    },
    {
        "id": 2,
