document.addEventListener('DOMContentLoaded', function() {
    ...
    function init() {
        fetch(metadataUrl)
            ...
            .then(function(data) {
                ...
                } catch(e) { console.error('[VISOR-PB]', e); showError(); }
            })
            .catch(function(e) { console.error('[VISOR-PB]', e); showError(); });
    }
    ...
    function setupBook() {
        ...
        requestAnimationFrame(function() {
            setTimeout(function() {
                try { initFlipBook(bookEl); } catch(e) { console.error('[VISOR-PB] flip init', e); }
            }, 150);
        });
    }
    ...
    function initFlipBook(bookEl) {
        ...
        probe.onerror = function() {
            buildFlipBook(bookEl, bookData.aspect_ratio || 0.707);
        };
    ...
    function getVisibleTexts() {
        ...
        if (mobile || !flipBook) {
            var p = bookData.pages[currentPage];
            if (p && p.narration) texts.push(p.narration);
        } else {
            if (currentPage === 0) {
                var c = bookData.pages[0];
                if (c && c.narration) texts.push(c.narration);
            } else {
                var li = currentPage, ri = currentPage + 1;
                if (li < totalPages && bookData.pages[li] && bookData.pages[li].narration) texts.push(bookData.pages[li].narration);
                if (ri < totalPages && bookData.pages[ri] && bookData.pages[ri].narration) texts.push(bookData.pages[ri].narration);
            }
        }
        return texts;
    }
